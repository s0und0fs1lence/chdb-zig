...
test "ChConn - initialization and cleanup" {
    const allocator = testing.allocator;
    const conn = try chdb.ChConn.init(allocator, "");
    defer conn.deinit();

    // Simple test - if init succeeded, we're good
...
test "ChConn - query with results" {
    ...
    const query_result = try conn.query(@constCast("SELECT * FROM test ORDER BY id"), .{});
    var count: usize = 0;
...
test "ChConn - query with parameters" {
    ...
    const query_result = try conn.query(@constCast("SELECT * FROM test WHERE id = {i}"), .{1});
